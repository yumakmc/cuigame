...
void RogueGame::Update() {
	...
	case S_ChoosingTarget://‘Šè‚Ì‘ÎÛ‚ğ‘I‘ğ’†
		...
		break;
	case S_OtherTurn://‘Šè‚Ì‘ÎÛ‚ğ‘I‘ğ’†
		//‘¼‚Ìƒ^[ƒ“‚Ìs“®“ü‚ê‚éB
...
int RogueGame::Act(Chara *from,Chara *to,const Action type) {
	...
	if (ChooseNextPlayer()) {//‘SˆõI‚í‚ç‚È‚©‚Á‚½‚ç
		assert(false);//¡‚Ì‚Æ‚±—ˆ‚é‚Í‚¸‚È‚¢
		*situation = S_ChoosingAction;
	}
	else {
...
int RogueGame::SelectAction(const Action type) {
	myparty.GetMember(nowplayernum)->defending = false;//“ÁêŒø‰Ê‰ğœ
	switch (type) {
	case A_Attack:
		nowaction = A_Attack;
